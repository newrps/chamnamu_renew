# 폴리곤 단순화 / 렌더링 성능 작업 (2026-08-19, 2026-09-03 갱신)

> **2026-09-03**: 아래 "개수 기반 LOD"(`lod_for_count`, `ST_Simplify`)는 **제거됨**.
> `get_polygons_in_bbox`는 이제 항상 원본 geometry를 반환하고, `MAX_ROW_LIMIT`(4만)만
> 안전장치로 유지한다. `X-Simplify-Tolerance` 헤더는 프론트 캐시 키 포맷 호환을 위해
> 남겨뒀지만 값은 항상 `0`. 이하 히스토리/이슈 섹션은 제거 전 상태 기록이다.
